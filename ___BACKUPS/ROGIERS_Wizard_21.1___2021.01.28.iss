...
SetupIconFile=C:\ROGIERS INSTALLER SCRIPTING\INNO SCRIPT\GRAPHICS\ROGIERS_LOGO.ico
AppCopyright=Ⓒ Daan Verhoost
DisableDirPage=yes
AllowRootDirectory=True
Uninstallable=no
ChangesEnvironment=True
WizardImageFile=C:\ROGIERS INSTALLER SCRIPTING\INNO SCRIPT\GRAPHICS\ROGIERS_WELCOME.bmp
WizardSmallImageFile=C:\ROGIERS INSTALLER SCRIPTING\INNO SCRIPT\GRAPHICS\InstallWizardSmall.bmp
DisableWelcomePage=False
OutputDir=C:\ROGIERS INSTALLER SCRIPTING\INNO SCRIPT\OUTPUT
...
Name: "InstallatieTotalCommander"; Description: "Installatie Total Commander"; GroupDescription: "Standaard :"; Components: Volledig Beperkt
Name: "ScriptsUitvoeren"; Description: "Powershell scripts uitvoeren [visuele effecten, snelkoppelingen, handleidingen kopiëren, ...]"; GroupDescription: "Standaard :"; Components: Volledig Beperkt
...
; Files kopiëren naar TEMP folder :
...
Source: "DATA\INSTALL\ROGIERS Installer\SCRIPTS\13___DisableWindowAnimation.ps1"; DestDir: "{tmp}"; Permissions: everyone-full
...
Source: "DATA\INSTALL\ROGIERS Installer\SCRIPTS\12___ChangeDisplayLanguage.ps1"; DestDir: "{tmp}"; Permissions: everyone-full

; ________________________________________________________________________________________________
; Chocolatey packages :

; InstallPackage = Performs the basic install of the Chocolatey package manager + installation of most used apps (see ps1 script) 
...
; INSTALLATIE TOTAL COMMANDER [SILENT MODE] :
; -----------------------------------------------------------------------------------
...
; PS1 SCRIPTS UITVOEREN OM WINDOWS SETTINGS AAN TE PASSEN :
; -----------------------------------------------------------------------------------
...
Filename: "powershell.exe"; \
    Parameters: "-ExecutionPolicy Bypass -File ""{tmp}\13___DisableWindowAnimation.ps1"""; \
    WorkingDir: "{app}"; \
    Flags: runhidden; \
    Components: Beperkt Volledig; \
    Tasks: ScriptsUitvoeren
...
; PS1 SCRIPTS VOOR PACKAGE MANAGER "CHOCOLATEY" :
; -----------------------------------------------------------------------------------
...
[ThirdParty]
CompileLogFile=C:\ROGIERS INSTALLER SCRIPTING\INNO SCRIPT\DATA\INSTALL\ROGIERS Installer\Install_log.txt
...
{_______________________________________________________________________________________________________________________________}
      ...
      { Uitlezen van datum voor verder gebruik in aanmaak van mapnaam }
      function DateTime: String;
        begin
            Result := GetDateTimeString('yyyy/mm/dd', '.', ':');
        end;        

      { Ingave van gebruiken omzetten in 'String' om bij aanmaak van map opnieuw te gebruiken. }
...
{_______________________________________________________________________________________________________________________________}
        ...
        { Start procedure voor aanmaak nieuwe pagina }
...
begin
  ...
  { Nieuwe pagina toevoegen na 'selectie van opdrachten' voor taalwijziging }
CustomLanguagePage:= CreateCustomPage(wpSelectTasks, 'Windows weergavetaal & toetsenbordindeling', 'Selecteer één van onderstaande opties');
...
begin
  if CurPageID = CustomLanguagePage.ID then
  begin      
    if CheckListBox.Checked[NW] then
    begin
      MsgBox('Niet wijzigen', mbInformation, MB_OK);
      {Zal niets uitvoeren bij uitvoeren van de installer}
    end;
    if CheckListBox.Checked[NL1] then
    begin
      MsgBox('Windows taal wijzigen: NEDERLANDS - QWERTY toetsenbord (SCM origineel)', mbInformation, MB_OK);
    end;
    if CheckListBox.Checked[NL2] then
    begin
      MsgBox('Windows taal wijzigen: NEDERLANDS - AZERTY toetsenbord', mbInformation, MB_OK);
    end;
    if CheckListBox.Checked[FR1] then
    begin
      MsgBox('Windows taal wijzigen: FRANS - QWERTY toetsenbord (SCM origineel)', mbInformation, MB_OK);
      ...
      {Run the powershell script here?}
                ...
                begin
                      if not ShellExec('', ExpandConstant('{app}\DATA\INSTALL\ROGIERS Installer\SCRIPTS\12___ChangeDisplayLanguage.ps1'),
    ...
    begin
      MsgBox('Windows taal wijzigen: FRANS - AZERTY toetsenbord', mbInformation, MB_OK);
    end;
...
{ Nieuwe pagina toevoegen na 'Welcome' page }
  begin
  AddCustomQueryPage();  

      {     if UserInput = '' then
          begin
            MsgBox('Gelieve een naam in te vullen.', mbError, MB_OK);
            end;               }

...
OutputBaseFilename=ROGIERS Wizard 23.0
...
WizardImageFile=.\GRAPHICS\ROGIERS_WELCOME_v23.bmp
...
[Tasks]
Name: "ChangeWallpaperSCM"; Description: "ROGIERS Wallpaper instellen - SCM Group"; GroupDescription: "Wallpaper :"; Components: Volledig Beperkt
Name: "ChangeWallpaperFOM"; Description: "ROGIERS Wallpaper instellen - FOM Industrie"; GroupDescription: "Wallpaper :"; Flags: unchecked; Components: Volledig Beperkt
...
Source: "DATA\INSTALL\ROGIERS Wizard\SCRIPTS\08___ChangeWallpaperRogiers_SCM.ps1"; DestDir: "{tmp}"; Permissions: everyone-full
Source: "DATA\INSTALL\ROGIERS Wizard\SCRIPTS\08___ChangeWallpaperRogiers_FOM.ps1"; DestDir: "{tmp}"; Permissions: everyone-full
...
Filename: "powershell.exe"; \
    Parameters: "-ExecutionPolicy Bypass -File ""{tmp}\08___ChangeWallpaperRogiers_SCM.ps1"""; \
    WorkingDir: "{app}"; \
    Flags: runhidden; \
    Components: Beperkt Volledig; \
    Tasks: ChangeWallpaperSCM

Filename: "powershell.exe"; \
    Parameters: "-ExecutionPolicy Bypass -File ""{tmp}\08___ChangeWallpaperRogiers_FOM.ps1"""; \
    WorkingDir: "{app}"; \
    Flags: runhidden; \
    Components: Beperkt Volledig; \
    Tasks: ChangeWallpaperFOM
...
Filename: "powershell.exe"; \
    ...
    Flags: runhidden; \
    Components: Beperkt Volledig; \
    Tasks: MapNetworkDrives

; Shortcut op bureaublad plaatsen - CleanPrintSpooler :
Filename: "powershell.exe"; \
    Parameters: "-ExecutionPolicy Bypass -File ""{tmp}\18___CleanPrintSpoolerShortcut.ps1"""; \
    WorkingDir: "{app}"; \
    Flags: runhidden; \
    Components: Beperkt Volledig; \
...
{ Prevent user from typing spaces in the PC NAME textbox }
...
function ValidateInputPCName(Sender: TWizardPage): Boolean;
begin
  Result := True;
 ...
 { Prevent user from leaving field blank on page "CustomInputPageTechnicianName" }
function ValidateNameInput(Sender: TWizardPage): Boolean;
begin
  Result := True;

  if Trim(CustomQueryPage.Values[0]) = '' then
  begin
    MsgBox('Vul de initialen van uw naam in. Dit veld mag niet leeg zijn.', mbError, MB_OK);
    Result := False;
  end;
end;
...
begin
  CustomQueryPage := CreateInputQueryPage(wpWelcome,
    ...
    'Dit wordt gebruikt om een map te creëren in "C:\ROGIERS\BACKUPS" op datum van vandaag. Vervolgens wordt een map gemaakt zoals : "2023.05.04 DVH". Plaats nadien hierin uw DAGRAPPORT.');

{ Input valideren }
CustomQueryPage.OnNextButtonClick := @ValidateNameInput;
...
CustomQueryPageHostname.OnNextButtonClick := @ValidateInputPCName;
{ Add items (False means it's not a password edit) }
